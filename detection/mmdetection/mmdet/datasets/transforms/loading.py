...
class LoadCTCImageFromFile(object):
    ...
    def __call__(self, results):
        ...
        assert not(self.fl_input == True and self.blue_input == True)

        filename = results['img_path']
        fluorescence_filename = results['fluorescence_img_path']
        ...
        if(self.fl_input):
            if(self.normalize):
                norm_fluorescence_img = cv2.normalize(fluorescence_img, None, alpha=0, beta=1, 
                    norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F)
                fluorescence_img = (255*norm_fluorescence_img).astype(np.uint8)

            img = np.concatenate([img, fluorescence_img], axis = 2)
        elif(self.blue_input):
            if(self.normalize):
                # print("in")
                fluorescence_img = fluorescence_img[:, :, 0][..., None]
                norm_fluorescence_img = cv2.normalize(fluorescence_img, None, alpha=0, beta=1, 
                    norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F)
                # print(fluorescence_img.mean())
                fluorescence_img = (255*norm_fluorescence_img).astype(np.uint8)
                # print(fluorescence_img.mean())
                img[..., 2] = fluorescence_img.copy()
                # img = np.concatenate([img, fluorescence_img[..., None]], axis = 2)
            else:
                fluorescence_img = fluorescence_img[:, :, 0][..., None]
                img = np.concatenate([img, fluorescence_img], axis = 2)
        else:
            pass
        # print(img.shape, self.blue_input)
        if self.to_float32:
            img = img.astype(np.float32)
